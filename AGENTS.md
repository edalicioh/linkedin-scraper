...
- Install with `npm ci`. Puppeteer's install step may download a browser.
...
- End-to-end checks require credentials, network access, a Puppeteer browser, and interaction with live LinkedIn markup; CAPTCHA or 2FA may intervene. Do not run them or claim scraper correctness without making that external interaction explicit.
...
- `src/core/browser.js` owns one shared Puppeteer instance. The scraper currently does not close it, and concurrent API requests share that browser without a concurrency guard.
